...
function blackFlag(array) {
    ...
    let [days, dailyLoot, target] = array.map(Number);
    let totalLoot = 0;
    ...
    for (let day = 1; day <= days; day++) {
        ...
        if (day % 3 === 0) {
            totalLoot += dailyLoot * 0.5;
        } 
        
        if (day % 5 === 0) {
            totalLoot -= totalLoot * 0.3;
        }

    }

    if (totalLoot < target) {
        totalLoot = Math.abs(totalLoot / target * 100);
        console.log(`Collected only ${totalLoot.toFixed(2)}% of the plunder.`);
    } else {
